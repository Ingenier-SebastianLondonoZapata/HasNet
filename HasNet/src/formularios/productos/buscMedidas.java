...
public class buscMedidas extends javax.swing.JDialog {
    ...
    public buscMedidas(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        this.tipo = tipo;
    ...
    private void initComponents() {
        ...
        btnRegistrar.setFont(new java.awt.Font("Century Gothic", 0, 11)); // NOI18N
    ...
    private void btnRegistrarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRegistrarActionPerformed
        if (txtId.getText().equals("")) {
            metodos.msgError(null, "Debe ingresar una medida");
            return;
        }
        
        String aux = instancias.getSql().getDatosMedidas(txtId.getText());
        if (aux == null) {
            if (metodos.msgPregunta(null, "¿Agregar medida?") == 0) {
                
                Boolean detalle = false;
                if (metodos.msgPregunta(null, "¿La medida es solo una unidad?") == 0) {
                    detalle = false;
                } else {
                    detalle = true;
                }
                
                if (!instancias.getSql().agregarMedida(txtId.getText(), detalle)) {
                    metodos.msgError(null, "Hubo un error al ingresar la medida");
                    return;
                }
                
                metodos.msgExito(null, "Medida ingresada con exito");
    ...
    public void setInstancia(Instancias instancia) {
        ...
        String condicion = "";
        if (instancia.getConfiguraciones().isFacturaElectronica()) {
            condicion = " where detalle = 1; ";
        }
        
        tblRegistros.setModel(instancias.getSql().getRegistrosMedidas(condicion));
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(buscMedidas.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                buscMedidas dialog = new buscMedidas(new javax.swing.JFrame(), true);
