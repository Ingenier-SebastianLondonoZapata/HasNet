...
public class buscColores extends javax.swing.JDialog {
    ...
    public buscColores(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    private void initComponents() {
        ...
        btnEste1.setFont(new java.awt.Font("Century Gothic", 1, 12)); // NOI18N
    ...
    private void btnEste1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEste1ActionPerformed
        if (txtId.getText().equals("")) {
            metodos.msgError(null, "Ingrese el color");
            return;
        }

        String aux = instancias.getSql().getDatosColores(txtId.getText().toUpperCase());

        if (aux == null) {
            if (metodos.msgPregunta(null, "¿Desea agregar el color ?") == 0) {

                if (!instancias.getSql().agregarColor(txtId.getText().toUpperCase())) {
                    metodos.msgError(null, "Hubo un error al ingresar la medida");
                    return;
                }

                metodos.msgExito(null, "Color ingresado con exito");
                this.setInstancia(instancias);
                noEncontrado(txtId.getText());
                btnEsteActionPerformed(evt);
            }
        } else {
            metodos.msgError(null, "El color ya existe");
        }
    ...
    public void setInstancia(Instancias instancia) {
        ...
        tblRegistros.setModel(instancias.getSql().getRegistrosColores());
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(buscColores.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                buscColores dialog = new buscColores(new javax.swing.JFrame(), true);
