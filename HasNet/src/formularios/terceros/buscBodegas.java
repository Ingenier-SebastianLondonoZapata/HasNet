...
public class buscBodegas extends javax.swing.JDialog {

    DefaultTableModel modelo;
    TableRowSorter modeloOrdenado;
    Instancias instancias;

    public buscBodegas(java.awt.Frame parent, boolean modal, String tipo) {
        super(parent, modal);
        initComponents();
        ...
        if (tipo.equals("INTERNA")) {
            consulta = " where tipo = 'INTERNA'  ";
        } else if (tipo.equals("PRINCIPAL")) {
            consulta = " where codigo = '123-22'  ";
        } else if (tipo.equals("PRESTAMO")) {
            consulta = " where codigo = '123-22' OR tipo <> 'INTERNA' AND codigo <> '123-11'  ";
        } else {
            consulta = " where tipo <> 'INTERNA' AND codigo <> '123-11' ";
        }

        tblRegistros.setModel(instancias.getSql().getRegistrosBodegas(consulta));
        ...
        if (tipo.equals("INTERNA")) {
            if (!instancias.getConfiguraciones().isInventarioBodegas()) {
                while (tblRegistros.getRowCount() > 1) {
                    modelo.removeRow(1);
                }
            }
        }
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                buscBodegas dialog = new buscBodegas(new javax.swing.JFrame(), true, "");
